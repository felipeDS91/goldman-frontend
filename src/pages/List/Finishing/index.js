...
export default function ListFinishing() {
  ...
  async function loadData(searchBy, pageNumber = 1) {
    ...
    try {
      const result = await api.get('/finishings', {
        params: {
          page: pageNumber,
          q: searchBy,
        },
  ...
  async function deleteRegister({ id, description }) {
    ...
    if (confirm.value) {
      try {
        await api.delete(`finishings/${id}`);
        await loadData();
  ...
  return (
    <Container>
      <PageHeader>
        <TitlePage>Gerenciando formatos externos</TitlePage>
        <Options>
          <Link to="/finishing">
          ...
          {data.length > 0 ? (
            data.map(item => (
              <tr key={item.id}>
                ...
                <td align="center">
                  <EditButton to={`/finishing/${item.id}`}>editar</EditButton>
                </td>
