...
export default function ListFreightType() {
  ...
  async function loadData(searchBy, pageNumber = 1) {
    ...
    try {
      const result = await api.get('/freight-types', {
        params: {
          page: pageNumber,
          q: searchBy,
        },
  ...
  async function deleteRegister({ id, description }) {
    ...
    if (confirm.value) {
      try {
        await api.delete(`freight-types/${id}`);
        await loadData();
  ...
  return (
    <Container>
      <PageHeader>
        <TitlePage>Gerenciando tipo de frete</TitlePage>
        <Options>
          <Link to="/freight-type">
          ...
          {data.length > 0 ? (
            data.map(item => (
              <tr key={item.id}>
                ...
                <td align="center">
                  <EditButton to={`/freight-type/${item.id}`}>
                    editar
                  </EditButton>
                </td>
