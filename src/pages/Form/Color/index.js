...
export default function FormColor({ match }) {
  ...
  const handleSubmit = useCallback(
    async sendData => {
      try {
        if (editMode) {
          await api.put(`colors/${id}`, sendData);
        } else {
          await api.post('colors', sendData);
        }

        ToastSuccess('Cadastro salvo com sucesso');
        history.push('/list-colors');
      } catch ({ response }) {
        const msg =
  ...
  return (
    <Container>
      <PageHeader>
        <TitlePage>{editMode ? 'Edição' : 'Cadastro'} de tonalidade</TitlePage>
        <Options>
